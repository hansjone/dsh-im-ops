...
// plugin-src/client/default-model.js
var React19 = __toESM(require("react"), 1);

// src/channels/shared/default-model.mjs
...
function modelCatalogItems(catalog) {
  ...
  for (const group of Array.isArray(catalog?.groups) ? catalog.groups : []) {
    ...
    for (const model of group.models) {
      ...
      seen.add(key);
      const label = typeof model.name === "string" && model.name.trim() ? model.name.trim() : modelId;
      items.push({
        provider,
        model: modelId,
        key,
...
// plugin-src/client/default-model.js
var SET_DEFAULT_MODEL_ENDPOINT = "bot.model.set";
var EMPTY_MODEL_CATALOG = Object.freeze({
  groups: Object.freeze([]),
  failures: Object.freeze([]),
  items: Object.freeze([])
});
var ModelCatalogContext = React19.createContext(EMPTY_MODEL_CATALOG);
function normalizeDefaultModelSelection(value) {
  return normalizeDefaultModel(value);
}
function normalizeHostModelCatalog(value) {
  return normalizeModelCatalog(value);
}
function DefaultModelEditor({ defaultModel = null, disabled = false, onSave }) {
  const catalog = React19.useContext(ModelCatalogContext) ?? EMPTY_MODEL_CATALOG;
  const helpId = React19.useId();
  const current = normalizeDefaultModel(defaultModel);
  const currentKey = defaultModelKey(current);
  const [saving, setSaving] = React19.useState(false);
...
function WhatsappSettingsTab({ rpcCall }) {
  ...
  const botList = model.bots.length > 0 ? h2(
    ...
    h2("ul", { className: "ddt-list dim-botList" }, model.bots.map((account) => h2("li", { key: account.botId }, h2(WhatsappAccountCard, {
      ...
      onDefaultModelSave: (defaultModel) => botAction(
        ...
        { botId: account.botId, defaultModel }
      ),
      onContextEnhancementSave: (config) => botAction(
        account,
        "context-enhancement",
  ...
  return h2(AgentPresetCatalogContext.Provider, {
    value: model.agentPresetCatalog ?? EMPTY_AGENT_PRESET_CATALOG
  }, h2(ModelCatalogContext.Provider, {
...
function ChannelPeerLabel({ sessionId, resolveChannelPeer }) {
  const [peer, setPeer] = React21.useState(null);
  React21.useEffect(() => {
...
function PhoneTypeahead({
  ...
  const [query, setQuery] = React23.useState(value || "");
  const [open, setOpen] = React23.useState(false);
  const blurTimer = React23.useRef(null);
  React23.useEffect(() => {
    setQuery(value || "");
  }, [value]);
  React23.useEffect(() => () => {
...
function AgentPresetSelect({
  ...
  return h2(
    ...
    h2("option", { value: "" }, localizeText("\u8DDF\u968F\u9891\u9053\u6620\u5C04")),
...
function AccessGrantSettingsPage({ channel: channel4, account, rpcCall, onSaved }) {
  ...
  const initialScope = normalizeGroupSessionScope(account?.groupSessionScope);
  const [draft, setDraft] = React23.useState(() => cloneGrant(initialGrant, ownerPhone));
  const [groupSessionScope, setGroupSessionScope] = React23.useState(initialScope);
  const [saving, setSaving] = React23.useState(false);
  const [feedback, setFeedback] = React23.useState(null);
  const [newGroupJid, setNewGroupJid] = React23.useState("");
  const [loadingGrant, setLoadingGrant] = React23.useState(!initialGrant);
  const contextCatalog = React23.useContext(AgentPresetCatalogContext);
  const [presetCatalog, setPresetCatalog] = React23.useState(() => normalizeAgentPresetCatalog(
  ...
  const knownGroupJids = React23.useMemo(() => {
  ...
  const actionableContacts = React23.useMemo(
      ...
      suggestions.map((suggestion, index) => {
        ...
        return h2(
          ...
          h2(
            ...
            React24.createElement("code", null, fieldsFor(definition, suggestion.kind).map((field) => maskRouteValue(suggestion.route?.[field.key])).filter(Boolean).join(" \xB7 "))
          )
        );
      })
    ),
...
function DeliveryTargetSettingsPage({
  ...
  const [accessGrant, setAccessGrant] = React24.useState(account.accessGrant);
...
function ManualUpdateCommand({ command, disabled, sourceInstall, desktop }) {
  const [copyState, setCopyState] = React25.useState("idle");
  const commandRef = React25.useRef(null);
  const mounted = React25.useRef(false);
  const copying = React25.useRef(false);
  React25.useEffect(() => {
    mounted.current = true;
    return () => {
      mounted.current = false;
...
function UpdateDialog({ children, onClose }) {
  ...
  const descriptionId = React25.useId();
  React25.useEffect(() => {
...
function UpdatePanel({ rpcCall, clientVersion, onStatus }) {
  const [snapshot, setSnapshot] = React25.useState(null);
  const [action, setAction] = React25.useState("status");
  const [error, setError] = React25.useState(null);
  const [open, setOpen] = React25.useState(false);
  const [uncertainInstall, setUncertainInstall] = React25.useState(false);
  const mounted = React25.useRef(false);
  const busy = React25.useRef(false);
  const readController = React25.useRef(null);
  const pollReadController = React25.useRef(null);
  const installRequest = React25.useRef(null);
  const onStatusRef = React25.useRef(onStatus);
  onStatusRef.current = onStatus;
  const accept = React25.useCallback((next) => {
    setSnapshot(next);
...
function IMSettingsTab({
  ...
  const [selected, setSelected] = React26.useState("weixin");
  const [loopbackRecovery, setLoopbackRecovery] = React26.useState(null);
  const [runningVersion, setRunningVersion] = React26.useState(IM_PLUGIN_VERSION);
  const [deliverySettings, setDeliverySettings] = React26.useState(null);
  const githubTooltipId = React26.useId();
  const channelSelectId = React26.useId();
  const active = CHANNELS.find((channel4) => channel4.id === selected) ?? CHANNELS[0];
  const reportLoopbackRecovery = React26.useCallback((recovery) => {
    setLoopbackRecovery((current) => current?.url === recovery.url ? current : recovery);
  }, []);
  const reportUpdateStatus = React26.useCallback((snapshot) => {
    setRunningVersion(snapshot.runningVersion);
  }, []);
  const rpcCalls = React26.useMemo(() => createLoopbackAwareRpcCalls({
    dingtalkRpcCall,
    discordRpcCall,
    feishuRpcCall,
    qqRpcCall,
  ...
  const botSettingsContext = React26.useMemo(() => Object.freeze({
