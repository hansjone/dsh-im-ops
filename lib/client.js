...
var package_default = {
  ...
  repository: {
    type: "git",
    url: "git+https://github.com/hansjone/dsh-im-ops.git"
  },
  homepage: "https://github.com/hansjone/dsh-im-ops#readme",
  bugs: {
    url: "https://github.com/hansjone/dsh-im-ops/issues"
  },
  bin: {
...
var EN = Object.freeze({
  ...
  "\u4F1A\u5217\u51FA\u5DF2\u804A\u8FC7\u7684\u79C1\u804A/\u7FA4\u804A\uFF0C\u4EE5\u53CA\u8BBF\u95EE\u540D\u5355\u91CC\u8BB0\u4F4F\u7684\u8054\u7CFB\u4EBA\u4E0E\u7FA4\uFF08\u663E\u793A\u4E3A\u6635\u79F0 \xB7 \u7535\u8BDD / \u7FA4\u540D\uFF09\u3002": "Lists known DMs and groups, plus remembered access-list contacts and groups (shown as nickname \xB7 phone / group name).",
  "\u53EF\u9009\u6295\u9012\u4F1A\u8BDD": "Available delivery conversations",
  "\u4F8B\u5982 \u5F20\u4E09 \xB7 13800138000": "For example, Alice \xB7 13800138000",
  "\u6B63\u5728\u5237\u65B0\u2026": "Refreshing\u2026",
  "\u5237\u65B0": "Refresh",
  ...
  "\u5148\u5728\u5BF9\u5E94\u5E73\u53F0\u4E0E\u673A\u5668\u4EBA\u804A\u4E00\u6761\u6D88\u606F\uFF08\u79C1\u804A\u6216\u7FA4\u804A\uFF09\uFF0C\u6216\u5148\u5728\u300C\u8BBF\u95EE\u8BBE\u7F6E\u300D\u91CC\u51FA\u73B0\u8FC7\u8054\u7CFB\u4EBA/\u7FA4\uFF0C\u518D\u5237\u65B0\u3002": "Chat with the bot first (DM or group), or wait until contacts/groups appear under Access settings, then refresh.",
  "\u5DF2\u6DFB\u52A0": "Added",
...
function createTokenChannelApi(channel4, connectionSummary, {
  ...
  return Object.freeze({ unwrapRpcResult: unwrapRpcResult13, normalizeSnapshot: normalizeSnapshot9, presentError: presentError12 });
}
...
function normalizeOptionalAgentPresetId(value) {
  if (typeof value !== "string") return null;
  const id5 = value.trim();
  return AGENT_PRESET_ID.test(id5) ? id5 : null;
}
...
function optionalAgentPreset(value) {
  ...
  const id5 = normalizeOptionalAgentPresetId(value);
  if (!id5) invalidGrant("Agent Preset \u65E0\u6548\u3002");
  return id5;
}
...
// plugin-src/client/channel-peer-label.js
...
function ChannelPeerLabel({ sessionId, resolveChannelPeer }) {
  const [peer, setPeer] = React20.useState(null);
  React20.useEffect(() => {
  ...
  const channelName2 = CHANNEL_LABELS[peer.channel] || peer.channel || "IM";
  const title = peer.kind === "group" ? `${channelName2} \xB7 ${peer.label}` : `${channelName2} \xB7 ${peer.label}`;
  return h2(
    "span",
    {
      className: "dim-channelPeerLabel",
      title,
      "aria-label": localizeText("\u6E20\u9053\u7528\u6237") + ` \xB7 ${title}`
    },
    h2("span", { className: "dim-channelPeerChannel", "aria-hidden": "true" }, channelName2),
    h2("span", { className: "dim-channelPeerText" }, peer.label)
  );
}
async function resolveWhatsappChannelPeer(whatsappRpcCall, sessionId, signal) {
  ...
  const value = unwrapRpcResult9(result);
...
function ScenePolicyEditor({
  ...
  const ownerHelpId = React21.useId();
  const emptyAllowlistHelpId = React21.useId();
...
function AccessPolicySettingsPage({ channel: channel4, account, rpcCall, onSaved }) {
  ...
  const [draft, setDraft] = React21.useState(() => clonePolicy(
    initialPolicy ?? DEFAULT_ACCESS_POLICY
  ));
  const [groupSessionScope, setGroupSessionScope] = React21.useState(initialScope);
  const [saving, setSaving] = React21.useState(false);
  const [feedback, setFeedback] = React21.useState(null);
  React21.useEffect(() => {
...
var ACCESS_GROUP_TITLES_REFRESH_ENDPOINT = "bot.access-group-titles.refresh";
...
function contactLabel(contact) {
  const name2 = contact.pushName || "\u672A\u547D\u540D";
  const phone = contact.phone || "\u5F85\u8865\u7535\u8BDD";
  return `${name2} \xB7 ${phone}`;
}
...
function contactNeedsGrantAction(contact, draft, knownGroupJids) {
  ...
  const pendingGroups = (contact.groupJids ?? []).filter((jid) => knownGroupJids.includes(jid)).filter((groupJid) => {
    const group = draft.groups?.[groupJid] ?? { admins: [], members: [] };
    return !(group.admins ?? []).includes(phone) && !(group.members ?? []).some((m) => m.phone === phone);
  });
...
function PhoneTypeahead({
  ...
  const [query, setQuery] = React22.useState(value || "");
  const [open, setOpen] = React22.useState(false);
  const blurTimer = React22.useRef(null);
  React22.useEffect(() => {
    setQuery(value || "");
  }, [value]);
  React22.useEffect(() => () => {
  ...
  const suggestions = (contacts ?? []).filter((contact) => {
    ...
    return (contact.phone ?? "").includes(queryDigits) || (contact.pushName ?? "").toLowerCase().includes(q);
  }).slice(0, 8);
  const showSuggestions = open && !disabled && suggestions.length > 0;
  return h2(
    "div",
    { className: "dim-accessTypeahead" },
    h2("input", {
      value: query,
      disabled,
      placeholder,
    ...
    showSuggestions ? h2(
      "ul",
      { className: "dim-accessSuggestList", role: "listbox" },
      suggestions.map((contact) => h2(
        "li",
        { key: `${contact.phone}-${contact.lids?.[0] ?? ""}` },
        h2("button", {
          type: "button",
...
function AgentPresetSelect({
  ...
  return h2(
    ...
    h2("option", { value: "" }, "\u8DDF\u968F\u5168\u5C40"),
    ...items.map((item) => h2("option", {
      key: item.id,
      value: item.id
...
function MemberRows({
  ...
  return h2(
    ...
    nested ? h2("h4", { className: "dim-accessSubblockTitle" }, title) : h2("legend", null, title),
    h2(
      "div",
      { className: "dim-accessTableWrap" },
      h2(
        "table",
        {
        ...
        h2(
          ...
          h2(
            ...
            h2("th", { scope: "col" }, "\u6635\u79F0"),
            h2("th", { scope: "col" }, "\u7535\u8BDD"),
            h2("th", { scope: "col" }, "\u547D\u4EE4\u6743\u9650"),
            showAgentPreset ? h2("th", { scope: "col" }, "Agent Preset") : null,
            h2("th", { scope: "col", className: "dim-accessTableActions" }, "\u64CD\u4F5C")
          )
        ),
        h2("tbody", null, members.length === 0 ? h2("tr", null, h2("td", {
          colSpan: columns,
          className: "dim-accessTableEmpty"
        }, "\u6682\u65E0\u7528\u6237\uFF0C\u53EF\u70B9\u4E0B\u65B9\u65B0\u589E\u3002")) : members.map((member, index) => h2(
          "tr",
          { key: `member-${index}` },
          h2(
            "td",
            { className: "dim-accessTableNick" },
            h2(NicknameHint, { contacts, phone: member.phone, shortEmpty: true })
          ),
          h2(
            "td",
            { className: "dim-accessTablePhone" },
            h2(PhoneTypeahead, {
              value: member.phone,
              contacts,
              disabled,
              placeholder: "8613800000000",
              onChange: (phone) => onChange(members.map((entry, i) => i === index ? { ...entry, phone } : entry))
            })
          ),
          h2(
            "td",
            { className: "dim-accessTablePerm" },
            h2(
              "select",
              {
                className: "dim-accessCompactSelect",
                ...
                disabled,
                onChange: (event) => onChange(members.map((entry, i) => i === index ? { ...entry, canExecuteCommands: event.target.value === "allow" } : entry))
              },
              h2("option", { value: "allow" }, "\u53EF\u6267\u884C\u547D\u4EE4"),
              h2("option", { value: "deny" }, "\u4E0D\u53EF\u6267\u884C\u547D\u4EE4")
            )
          ),
          showAgentPreset ? h2(
            "td",
            { className: "dim-accessTablePreset" },
            h2(AgentPresetSelect, {
          ...
          h2(
            "td",
            { className: "dim-accessTableActions" },
            h2("button", {
              type: "button",
...
function AdminPhones({
  ...
  return h2(
    ...
    nested ? h2("h4", { className: "dim-accessSubblockTitle" }, title) : h2("legend", null, title),
    help ? h2("p", { className: "dim-accessHint" }, help) : null,
    h2(
      "div",
      { className: "dim-accessTableWrap" },
      h2(
        "table",
        { className: "dim-accessTable", "data-kind": "admin" },
        h2(
          "thead",
          null,
          h2(
        ...
        h2("tbody", null, phones.length === 0 ? h2("tr", null, h2("td", { colSpan: 3, className: "dim-accessTableEmpty" }, "\u6682\u65E0\u7BA1\u7406\u5458\uFF0C\u53EF\u70B9\u4E0B\u65B9\u65B0\u589E\u3002")) : phones.map((phone, index) => {
          ...
          return h2(
            ...
            h2(
              "td",
              { className: "dim-accessTableNick" },
              locked ? h2("span", { className: "dim-accessNickname" }, "\u7ED1\u5B9A\u8D26\u53F7") : h2(NicknameHint, { contacts, phone, shortEmpty: true })
            ),
            h2(
              "td",
              { className: "dim-accessTablePhone" },
            ...
            h2(
              "td",
              { className: "dim-accessTableActions" },
              locked ? h2("span", { className: "dim-accessTableLocked" }, "\u4E0D\u53EF\u5220\u9664") : h2("button", {
                type: "button",
                className: "dim-deliveryButton dim-accessCompactDelete",
                "data-kind": "danger",
                disabled,
...
function AccessGrantSettingsPage({ channel: channel4, account, rpcCall, onSaved }) {
  if (channel4 !== "whatsapp") {
    return h2(
      "div",
      { className: "dim-accessState", role: "alert" },
      "\u5F53\u524D\u6E20\u9053\u4ECD\u4F7F\u7528\u65E7\u7248\u8BBF\u95EE\u8BBE\u7F6E\u3002"
  ...
  const initialScope = normalizeGroupSessionScope(account?.groupSessionScope);
  const [draft, setDraft] = React22.useState(() => cloneGrant(initialGrant, ownerPhone));
  const [groupSessionScope, setGroupSessionScope] = React22.useState(initialScope);
  const [saving, setSaving] = React22.useState(false);
  const [feedback, setFeedback] = React22.useState(null);
  const [newGroupJid, setNewGroupJid] = React22.useState("");
  const [loadingGrant, setLoadingGrant] = React22.useState(!initialGrant);
  const contextCatalog = React22.useContext(AgentPresetCatalogContext);
  const [presetCatalog, setPresetCatalog] = React22.useState(() => normalizeAgentPresetCatalog(
  ...
  React22.useEffect(() => {
    if (typeof rpcCall !== "function" || !account?.botId) return void 0;
    let cancelled = false;
    setLoadingGrant(true);
    void (async () => {
      try {
        const value = unwrapRpcResult11(await rpcCall("connection.status", {}));
        if (cancelled) return;
        if (value?.agentPresetCatalog) {
          setPresetCatalog(normalizeAgentPresetCatalog(value.agentPresetCatalog));
        }
        const bot = Array.isArray(value?.bots) ? value.bots.find((entry) => entry?.botId === account.botId) : null;
        ...
        const missingTitles = Object.entries(grant?.groups ?? {}).filter(([, group]) => !(typeof group?.title === "string" && group.title.trim())).map(([jid]) => jid);
        const pendingGroupJids = (grant?.pending ?? []).map((entry) => entry.groupJid).filter((jid) => typeof jid === "string" && jid.endsWith("@g.us"));
        const toSync = [.../* @__PURE__ */ new Set([...missingTitles, ...pendingGroupJids])];
        if (toSync.length > 0) {
          try {
            const refreshed = unwrapRpcResult11(await rpcCall(ACCESS_GROUP_TITLES_REFRESH_ENDPOINT, {
              botId: account.botId,
              groupJids: toSync
            }));
            if (cancelled) return;
  ...
  const knownGroupJids = React22.useMemo(() => {
  ...
  const actionableContacts = React22.useMemo(
  ...
  const resolvePending = async (pendingId, action) => {
    ...
    try {
      ...
      const value = unwrapRpcResult11(await rpcCall(ACCESS_PENDING_RESOLVE_ENDPOINT, {
        botId: account.botId,
        pendingId,
        action,
        resolvedByPhone
  ...
  const save = async (event) => {
    ...
    setSaving(true);
    try {
      const admins = [...new Set(draft.globalAdmins.map((phone) => normalizeAccessPhone(phone)).filter(Boolean))];
      if (ownerPhone && !admins.includes(ownerPhone)) admins.unshift(ownerPhone);
      const grant = validateAccessGrant({
        ...draft,
        globalAdmins: admins,
        directMembers: draft.directMembers.map((m) => ({ ...m, phone: normalizeAccessPhone(m.phone) })).filter((m) => m.phone),
        groups: Object.fromEntries(Object.entries(draft.groups).map(([jid, group]) => [jid, {
          ...
          members: group.members.map((m) => ({ ...m, phone: normalizeAccessPhone(m.phone) })).filter((m) => m.phone)
        }]))
      });
      const scope = validateGroupSessionScope(groupSessionScope);
      if (typeof rpcCall !== "function") throw new Error("\u8BBF\u95EE\u6388\u6743\u6682\u4E0D\u53EF\u7528\u3002");
      const value = unwrapRpcResult11(await rpcCall(ACCESS_GRANT_ENDPOINT, {
        botId: account.botId,
        grant
      }));
      unwrapRpcResult11(await rpcCall(GROUP_SESSION_SCOPE_ENDPOINT2, {
        botId: account.botId,
        groupSessionScope: scope
      }));
  ...
  return h2(
    ...
    loadingGrant ? h2("div", { className: "dim-accessState", "aria-busy": "true" }, "\u6B63\u5728\u8BFB\u53D6\u5206\u7EA7\u8BBF\u95EE\u8BBE\u7F6E\u2026") : null,
    h2(
      "p",
    ...
    h2(
      ...
      h2("legend", null, "\u5F85\u5BA1\u6279"),
      (draft.pending ?? []).filter((entry) => entry.status === "pending" || !entry.status).length === 0 ? h2("div", { className: "dim-accessUsersEmpty" }, "\u6682\u65E0\u5F85\u6279\u7533\u8BF7") : h2(
        "ul",
        { className: "dim-accessUserList" },
        (draft.pending ?? []).filter((entry) => entry.status === "pending" || !entry.status).map((entry) => h2(
          "li",
          { key: entry.id, className: "dim-accessUserRow" },
          h2(
            "div",
            { className: "dim-accessField" },
            h2("span", null, ...entry.kind === "group" ? ["\u7FA4\u804A \xB7 ", groupDisplayName(entry.groupJid, draft.groups)] : ["\u79C1\u804A"]),
            h2("strong", null, entry.pushName || entry.phone || entry.lid || entry.id),
            entry.unresolved ? h2("span", null, "\uFF08\u7535\u8BDD\u672A\u89E3\u6790\uFF0C\u8BF7\u5148\u5728\u8054\u7CFB\u4EBA\u4E2D\u786E\u8BA4\uFF09") : null,
            entry.requestText ? h2("p", null, entry.requestText.slice(0, 120)) : null
          ),
          h2("button", {
            type: "button",
            className: "dim-deliveryButton",
    ...
    h2(
      ...
      knownGroupJids.length === 0 ? h2(
        ...
        return h2(
          "div",
          { key: groupJid, className: "dim-accessGroupCard" },
          h2(
            "div",
            { className: "dim-accessGroupHeading" },
            h2("h3", null, groupDisplayName(groupJid, draft.groups)),
            h2("code", { className: "dim-accessGroupJid" }, groupJid)
          ),
          h2(
            "div",
            { className: "dim-accessGroupTitleRow" },
            h2(
              "label",
              { className: "dim-accessField" },
              h2("span", null, "\u7FA4\u540D\u79F0"),
            ...
            h2("button", {
              ...
              onClick: () => void (async () => {
                ...
                setFeedback(null);
                try {
                  const refreshed = unwrapRpcResult11(await rpcCall(
                  ...
                  setFeedback({
                    ...
                    message: title ? ["\u5DF2\u540C\u6B65\u7FA4\u540D\uFF1A", title].join("") : "\u672A\u80FD\u4ECE WhatsApp \u8BFB\u53D6\u7FA4\u540D\uFF0C\u8BF7\u786E\u8BA4\u673A\u5668\u4EBA\u5728\u7EBF\u4E14\u4ECD\u5728\u8BE5\u7FA4\u3002"
                  });
                } catch (error) {
                  setFeedback({
                    tone: "error",
    ...
    h2(
      ...
      h2(
        "p",
        { className: "dim-accessHint" },
        "\u4EC5\u8BB0\u5F55\u79C1\u804A\u673A\u5668\u4EBA\uFF0C\u6216\u5728\u7FA4\u91CC @ \u673A\u5668\u4EBA\u7684\u4EBA\u3002\u5DF2\u6709\u5BF9\u5E94\u6743\u9650\u7684\u4EBA\u4F1A\u81EA\u52A8\u9690\u85CF\uFF1B\u7535\u8BDD\u662F\u552F\u4E00\u6388\u6743\u952E\uFF0C\u53EF\u4E00\u952E\u52A0\u5165\u79C1\u804A/\u672C\u7FA4\u6388\u6743\u3002"
      ),
      actionableContacts.length === 0 ? h2(
        "div",
        { className: "dim-accessUsersEmpty" },
        contacts.length === 0 ? "\u6682\u65E0\u8054\u7CFB\u4EBA\u3002\u6709\u4EBA\u79C1\u804A\u6216 @ \u673A\u5668\u4EBA\u540E\u4F1A\u51FA\u73B0\u5728\u6B64\u3002" : "\u5F53\u524D\u8054\u7CFB\u4EBA\u5747\u5DF2\u5177\u5907\u5BF9\u5E94\u6743\u9650\uFF0C\u65E0\u9700\u518D\u6388\u6743\u3002"
      ) : h2(
        "div",
        { className: "dim-accessTableWrap" },
        h2(
          "table",
          { className: "dim-accessTable", "data-kind": "contact" },
          h2(
            "thead",
            null,
            h2(
          ...
          h2("tbody", null, actionableContacts.slice(0, 30).map((contact) => {
            const phone = contact.phone || "";
            const nickname = typeof contact.pushName === "string" && contact.pushName.trim() ? contact.pushName.trim() : "\u6682\u65E0\u6635\u79F0";
            const alreadyDirect = Boolean(phone) && ((draft.directMembers ?? []).some((m) => m.phone === phone) || (draft.globalAdmins ?? []).includes(phone));
            const groupTargets = (contact.groupJids ?? []).filter((jid) => knownGroupJids.includes(jid)).filter((groupJid) => {
              if (!phone) return true;
              const group = draft.groups[groupJid] ?? { admins: [], members: [] };
              return !(group.admins ?? []).includes(phone) && !(group.members ?? []).some((m) => m.phone === phone);
            });
            return h2(
              ...
              h2(
                ...
                { className: "dim-accessTableNick" },
                h2("span", {
                  className: nickname === "\u6682\u65E0\u6635\u79F0" ? "dim-accessNickname dim-accessNicknameEmpty" : "dim-accessNickname"
                }, nickname)
              ),
              h2(
                "td",
                { className: "dim-accessTablePhone" },
                phone || h2("span", { className: "dim-accessNicknameEmpty" }, "\u5F85\u8865\u7535\u8BDD")
              ),
              h2(
                "td",
                { className: "dim-accessTableSource" },
                (contact.scenes ?? []).join(" / ") || "\u2014"
              ),
              h2(
                "td",
                { className: "dim-accessTableActions dim-accessContactActions" },
                phone && !alreadyDirect ? h2("button", {
                  type: "button",
                  className: "dim-deliveryButton",
                  "data-kind": "primary",
                  disabled: saving,
                ...
                ...groupTargets.map((groupJid) => {
                  const name2 = groupDisplayName(groupJid, draft.groups);
                  return h2("button", {
                    key: `add-${groupJid}`,
                    type: "button",
                    className: "dim-deliveryButton",
                    ...
                    onClick: () => {
                      ...
                      setFeedback({
                        tone: "success",
                        message: ["\u5DF2\u52A0\u5165\u7FA4\u6388\u6743\uFF1A", name2, "\u3002\u8BB0\u5F97\u70B9\u4FDD\u5B58\u3002"].join("")
                      });
                    }
                  }, ["\u52A0\u5165\u7FA4\uFF1A", name2].join(""));
                })
...
function TargetForm({
  ...
  const [targetId, setTargetId] = React23.useState(initialValue?.targetId ?? "");
  const [name2, setName] = React23.useState(initialValue?.name ?? "");
  const [kind, setKind] = React23.useState(initialKind);
  const [route, setRoute] = React23.useState(initialValue?.route ?? {});
  const [error, setError] = React23.useState(null);
  const [testing, setTesting] = React23.useState(false);
  const [testState, setTestState] = React23.useState(null);
  const currentTarget = () => {
    ...
    h2(
      ...
      h2(
        ...
        h2("input", {
          ...
          disabled: testing,
          maxLength: 80,
          placeholder: definition.label === "WhatsApp" ? "\u4F8B\u5982 \u5F20\u4E09 \xB7 13800138000" : "\u4F8B\u5982 \u6BCF\u65E5\u6C47\u62A5\u7FA4",
...
function TargetSuggestionPicker({
  ...
  const channelHint = definition.label === "WhatsApp" ? "\u4F1A\u5217\u51FA\u5DF2\u804A\u8FC7\u7684\u79C1\u804A/\u7FA4\u804A\uFF0C\u4EE5\u53CA\u8BBF\u95EE\u540D\u5355\u91CC\u8BB0\u4F4F\u7684\u8054\u7CFB\u4EBA\u4E0E\u7FA4\uFF08\u663E\u793A\u4E3A\u6635\u79F0 \xB7 \u7535\u8BDD / \u7FA4\u540D\uFF09\u3002" : "\u9009\u62E9\u540E\u4F1A\u81EA\u52A8\u586B\u5199\u76EE\u6807\u4FE1\u606F\u548C\u8C03\u7528\u522B\u540D\uFF0C\u786E\u8BA4\u540E\u518D\u4FDD\u5B58\u3002";
  return h2(
    "section",
    ...
    phase === "loading" ? h2("div", { className: "dim-targetSuggestionState", "aria-busy": "true" }, "\u6B63\u5728\u8BFB\u53D6\u5DF2\u804A\u4F1A\u8BDD\u2026") : phase === "error" ? h2(
      ...
      h2("p", null, "\u5148\u5728\u5BF9\u5E94\u5E73\u53F0\u4E0E\u673A\u5668\u4EBA\u804A\u4E00\u6761\u6D88\u606F\uFF08\u79C1\u804A\u6216\u7FA4\u804A\uFF09\uFF0C\u6216\u5148\u5728\u300C\u8BBF\u95EE\u8BBE\u7F6E\u300D\u91CC\u51FA\u73B0\u8FC7\u8054\u7CFB\u4EBA/\u7FA4\uFF0C\u518D\u5237\u65B0\u3002")
    ) : h2(
      "ul",
      { className: "dim-targetSuggestionList", role: "listbox", "aria-label": "\u53EF\u9009\u6295\u9012\u4F1A\u8BDD" },
        ...
        return h2(
          ...
          h2(
            "button",
            {
              ...
              role: "option",
              "aria-disabled": added ? "true" : void 0,
              disabled: added,
              onClick: () => {
                if (!added) onSelect(suggestion);
              }
            },
            ...
            React23.createElement(
              "strong",
              null,
              suggestionDisplayName(suggestion) || suggestionFallbackName(definition, suggestion)
            ),
            h2("span", null, [
              localizeText(kindLabel(definition, suggestion.kind)),
              added ? localizeText("\u5DF2\u6DFB\u52A0") : null
            ].filter(Boolean).join(" \xB7 ")),
            React23.createElement("code", null, fieldsFor(definition, suggestion.kind).map((field) => maskRouteValue(suggestion.route?.[field.key])).filter(Boolean).join(" \xB7 "))
          )
        );
      })
    ),
...
function TargetRow({ definition, target, botId, connected, rpcCall, onChanged, onEdit }) {
  const [action, setAction] = React23.useState(null);
  const [testState, setTestState] = React23.useState(null);
  const [copyState, setCopyState] = React23.useState(null);
  const [confirmDelete, setConfirmDelete] = React23.useState(false);
  const testTarget = async () => {
    setAction("test");
    setTestState(null);
    try {
  ...
  return h2(
    ...
    h2(
      ...
      h2(
        ...
        React23.createElement("strong", null, target.name || target.targetId),
...
function DeliveryTargetSettingsPage({
  ...
  const [activeTabId, setActiveTabId] = React23.useState(BOT_SETTINGS_TABS[0].id);
  const [phase, setPhase] = React23.useState("loading");
  const [targets, setTargets] = React23.useState([]);
  const [suggestionPhase, setSuggestionPhase] = React23.useState("idle");
  const [suggestions, setSuggestions] = React23.useState([]);
  const [suggestionError, setSuggestionError] = React23.useState(null);
  const [error, setError] = React23.useState(null);
  const [editor, setEditor] = React23.useState(null);
  const [saving, setSaving] = React23.useState(false);
  const [botCopyState, setBotCopyState] = React23.useState(null);
  const [accessPolicy, setAccessPolicy] = React23.useState(account.accessPolicy);
  const [accessGrant, setAccessGrant] = React23.useState(account.accessGrant);
  ...
  const loadTargets = React23.useCallback(async ({ signal, silent = false } = {}) => {
  ...
  const loadSuggestions = React23.useCallback(async () => {
  ...
  return h2(
    ...
    h2(
      ...
      activeTab.id === "access" ? channel4 === "whatsapp" ? h2(AccessGrantSettingsPage, {
        channel: channel4,
        account: {
          ...account,
          accessGrant,
...
.dim-channelPickerControl { min-width: 0; flex: 1 1 auto; display: grid; grid-template-columns: 30px minmax(0, 1fr); align-items: center; gap: 10px; max-width: 420px; padding: 4px 10px 4px 6px; border: 1px solid var(--dsw-alias-border-l2, #dfe1e5); border-radius: 10px; background: var(--dsw-alias-bg-layer-1, #fff); box-shadow: 0 1px 2px rgb(31 35 41 / 4%); }
.dim-channelPickerSelect { min-width: 0; width: 100%; height: 34px; padding: 0 4px 0 0; border: 0; border-radius: 6px; color: var(--dsw-alias-label-primary, #1f2329); background: transparent; font: inherit; font-size: 14px; font-weight: 650; cursor: pointer; color-scheme: inherit; }
.dim-channelPickerSelect:focus { outline: none; }
.dim-channelPickerSelect option, .dim-accessCompactSelect option, .dim-panel select option, .dim-page select option { color: var(--dsw-alias-label-primary, #1f2329); background-color: var(--dsw-alias-bg-layer-1, #fff); }
...
.dim-page select, .dim-panel select { color: var(--dsw-alias-label-primary, #1f2329); background-color: var(--dsw-alias-bg-layer-1, #fff); color-scheme: inherit; }
body[data-ds-dark-theme] .dim-page select, body[data-ds-dark-theme] .dim-panel select, body[data-ds-dark-theme] .dim-channelPickerSelect, body[data-ds-dark-theme] .dim-accessCompactSelect { color-scheme: dark; }
body[data-ds-dark-theme] .dim-page select option, body[data-ds-dark-theme] .dim-panel select option, body[data-ds-dark-theme] .dim-channelPickerSelect option, body[data-ds-dark-theme] .dim-accessCompactSelect option { color: var(--dsw-alias-label-primary, #f5f6f7); background-color: var(--dsw-alias-bg-layer-1, #1f2329); }
...
.dim-accessCompactRow { position: relative; z-index: 0; min-width: 0; display: grid; grid-template-columns: minmax(0, 1fr) 140px 64px; align-items: center; gap: 8px; margin: 0; padding: 7px 8px; border: 0; border-bottom: 1px solid var(--dsw-alias-border-l1, #eef0f3); border-radius: 0; background: transparent; }
...
.dim-accessCompactIdentity { min-width: 0; display: grid; gap: 2px; }
.dim-accessCompactIdentity .dim-accessTypeahead > input { height: 32px; font-size: 12px; color: var(--dsw-alias-label-primary, #1f2329); background: var(--dsw-alias-bg-layer-1, #fff); border-color: var(--dsw-alias-border-l2, #dfe1e5); }
.dim-accessCompactSelect, .dim-accessCompactSelectSlot { width: 100%; min-width: 0; height: 32px; }
.dim-accessCompactSelect { padding: 0 6px; border: 1px solid var(--dsw-alias-border-l2, #dfe1e5); border-radius: 7px; color: var(--dsw-alias-label-primary, #1f2329); background: var(--dsw-alias-bg-layer-1, #fff); font: inherit; font-size: 12px; color-scheme: inherit; }
...
.dim-accessCompactSelectSlot { display: block; }
...
.dim-accessTypeahead { position: relative; z-index: 1; min-width: 0; display: grid; gap: 0; }
.dim-accessTypeahead:focus-within { z-index: 6; }
...
.dim-accessSuggestList { position: absolute; z-index: 7; top: calc(100% + 4px); left: 0; right: 0; margin: 0; padding: 4px; list-style: none; display: grid; gap: 2px; max-height: 180px; overflow: auto; border: 1px solid var(--dsw-alias-border-l2, #dfe1e5); border-radius: 8px; background: var(--dsw-alias-bg-layer-1, #fff); box-shadow: 0 10px 28px color-mix(in srgb, #1f2329 16%, transparent); }
.dim-accessSuggestList li { min-width: 0; }
.dim-accessSuggestItem { width: 100%; min-height: 32px; padding: 6px 9px; border: 0; border-radius: 6px; background: transparent; color: var(--dsw-alias-label-primary, #1f2329); font: inherit; font-size: 12px; text-align: left; cursor: pointer; }
.dim-accessSuggestItem:hover, .dim-accessSuggestItem:focus-visible { background: var(--dsw-alias-bg-module-platform, #f7f8fa); }
.dim-accessSubblock { min-width: 0; display: grid; gap: 8px; margin-top: 12px; padding-top: 12px; border-top: 1px dashed var(--dsw-alias-border-l1, #eef0f3); }
.dim-accessSubblockTitle { margin: 0; color: var(--dsw-alias-label-primary, #1f2329); font-size: 13px; line-height: 20px; font-weight: 620; }
.dim-accessHint { margin: 0; color: var(--dsw-alias-label-tertiary, #8f959e); font-size: 11px; line-height: 17px; }
.dim-accessGroupCard { min-width: 0; display: grid; gap: 12px; margin-top: 12px; padding: 14px; border: 1px solid var(--dsw-alias-border-l1, #eef0f3); border-radius: 10px; background: var(--dsw-alias-bg-module-platform, #f7f8fa); }
...
function manualUpdateCommand(snapshot) {
  ...
  return `dsh plugin --profile ${profileArgument} add -w dsh-im-ops@${version}`;
}
function ManualUpdateCommand({ command, disabled, sourceInstall, desktop }) {
  const [copyState, setCopyState] = React24.useState("idle");
  const commandRef = React24.useRef(null);
  const mounted = React24.useRef(false);
  const copying = React24.useRef(false);
  React24.useEffect(() => {
    mounted.current = true;
    return () => {
      mounted.current = false;
  ...
  return h2(
    ...
    command ? h2(
      ...
      command.endsWith("dsh-im-ops@latest") ? h2(
        "p",
...
function UpdateDialog({ children, onClose }) {
  const dialogRef = React24.useRef(null);
  const titleId = React24.useId();
  const descriptionId = React24.useId();
  React24.useEffect(() => {
...
function UpdatePanel({ rpcCall, clientVersion, onStatus }) {
  const [snapshot, setSnapshot] = React24.useState(null);
  const [action, setAction] = React24.useState("status");
  const [error, setError] = React24.useState(null);
  const [open, setOpen] = React24.useState(false);
  const [uncertainInstall, setUncertainInstall] = React24.useState(false);
  const mounted = React24.useRef(false);
  const busy = React24.useRef(false);
  const readController = React24.useRef(null);
  const pollReadController = React24.useRef(null);
  const installRequest = React24.useRef(null);
  const onStatusRef = React24.useRef(onStatus);
  onStatusRef.current = onStatus;
  const accept = React24.useCallback((next) => {
    setSnapshot(next);
...
function IMSettingsTab({
  ...
  const [selected, setSelected] = React25.useState("weixin");
  const [loopbackRecovery, setLoopbackRecovery] = React25.useState(null);
  const [runningVersion, setRunningVersion] = React25.useState(IM_PLUGIN_VERSION);
  const [deliverySettings, setDeliverySettings] = React25.useState(null);
  const githubTooltipId = React25.useId();
  const channelSelectId = React25.useId();
  const active = CHANNELS.find((channel4) => channel4.id === selected) ?? CHANNELS[0];
  const reportLoopbackRecovery = React25.useCallback((recovery) => {
    setLoopbackRecovery((current) => current?.url === recovery.url ? current : recovery);
  }, []);
  const reportUpdateStatus = React25.useCallback((snapshot) => {
    setRunningVersion(snapshot.runningVersion);
  }, []);
  const rpcCalls = React25.useMemo(() => createLoopbackAwareRpcCalls({
    dingtalkRpcCall,
    discordRpcCall,
    feishuRpcCall,
    qqRpcCall,
  ...
  const botSettingsContext = React25.useMemo(() => Object.freeze({
...
function apply(ctx) {
  ...
  const resolveChannelPeer = (sessionId, signal) => resolveWhatsappChannelPeer(whatsappRpcCall, sessionId, signal);
