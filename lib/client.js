...
function PhoneTypeahead({
  ...
  const [query, setQuery] = React21.useState(value || "");
  const [open, setOpen] = React21.useState(false);
  const blurTimer = React21.useRef(null);
  React21.useEffect(() => {
    setQuery(value || "");
  }, [value]);
  React21.useEffect(() => () => {
...
function AccessGrantSettingsPage({ channel: channel4, account, rpcCall, onSaved }) {
  ...
  const initialScope = normalizeGroupSessionScope(account?.groupSessionScope);
  const [draft, setDraft] = React21.useState(() => cloneGrant(initialGrant, ownerPhone));
  ...
  const [newGroupJid, setNewGroupJid] = React21.useState("");
  const [loadingGrant, setLoadingGrant] = React21.useState(!initialGrant);
  const contextCatalog = React21.useContext(AgentPresetCatalogContext);
  const [presetCatalog, setPresetCatalog] = React21.useState(() => normalizeAgentPresetCatalog(
  ...
  const knownGroupJids = React21.useMemo(() => {
  ...
  const actionableContacts = React21.useMemo(
...
function TargetForm({
  ...
  const [targetId, setTargetId] = React22.useState(initialValue?.targetId ?? "");
  const [name2, setName] = React22.useState(initialValue?.name ?? "");
  const [kind, setKind] = React22.useState(initialKind);
  const [route, setRoute] = React22.useState(initialValue?.route ?? {});
  const [error, setError] = React22.useState(null);
  const [testing, setTesting] = React22.useState(false);
  const [testState, setTestState] = React22.useState(null);
  const currentTarget = () => {
    ...
    phase === "loading" ? h2("div", { className: "dim-targetSuggestionState", "aria-busy": "true" }, "\u6B63\u5728\u8BFB\u53D6\u5DF2\u804A\u4F1A\u8BDD\u2026") : phase === "error" ? h2(
      ...
      h2(
        ...
        suggestions.map((suggestion, index) => {
          ...
          return React22.createElement("option", {
...
function TargetRow({ definition, target, botId, connected, rpcCall, onChanged, onEdit }) {
  const [action, setAction] = React22.useState(null);
  const [testState, setTestState] = React22.useState(null);
  const [copyState, setCopyState] = React22.useState(null);
  const [confirmDelete, setConfirmDelete] = React22.useState(false);
  const testTarget = async () => {
    setAction("test");
    setTestState(null);
    try {
  ...
  return h2(
    ...
    h2(
      ...
      h2(
        ...
        React22.createElement("strong", null, target.name || target.targetId),
...
function DeliveryTargetSettingsPage({
  ...
  const [activeTabId, setActiveTabId] = React22.useState(BOT_SETTINGS_TABS[0].id);
  const [phase, setPhase] = React22.useState("loading");
  const [targets, setTargets] = React22.useState([]);
  const [suggestionPhase, setSuggestionPhase] = React22.useState("idle");
  const [suggestions, setSuggestions] = React22.useState([]);
  const [suggestionError, setSuggestionError] = React22.useState(null);
  const [error, setError] = React22.useState(null);
  const [editor, setEditor] = React22.useState(null);
  const [saving, setSaving] = React22.useState(false);
  const [botCopyState, setBotCopyState] = React22.useState(null);
  const [accessPolicy, setAccessPolicy] = React22.useState(account.accessPolicy);
  const [accessGrant, setAccessGrant] = React22.useState(account.accessGrant);
  ...
  const loadTargets = React22.useCallback(async ({ signal, silent = false } = {}) => {
  ...
  const loadSuggestions = React22.useCallback(async () => {
...
function ManualUpdateCommand({ command, disabled, sourceInstall, desktop }) {
  const [copyState, setCopyState] = React23.useState("idle");
  const commandRef = React23.useRef(null);
  const mounted = React23.useRef(false);
  const copying = React23.useRef(false);
  React23.useEffect(() => {
    mounted.current = true;
    return () => {
      mounted.current = false;
...
function UpdateDialog({ children, onClose }) {
  const dialogRef = React23.useRef(null);
...
function UpdatePanel({ rpcCall, clientVersion, onStatus }) {
  const [snapshot, setSnapshot] = React23.useState(null);
  const [action, setAction] = React23.useState("status");
  const [error, setError] = React23.useState(null);
  const [open, setOpen] = React23.useState(false);
  const [uncertainInstall, setUncertainInstall] = React23.useState(false);
  const mounted = React23.useRef(false);
  const busy = React23.useRef(false);
  const readController = React23.useRef(null);
  const pollReadController = React23.useRef(null);
  const installRequest = React23.useRef(null);
  const onStatusRef = React23.useRef(onStatus);
  onStatusRef.current = onStatus;
  const accept = React23.useCallback((next) => {
    setSnapshot(next);
...
function IMSettingsTab({
  ...
  const [selected, setSelected] = React24.useState("weixin");
  const [loopbackRecovery, setLoopbackRecovery] = React24.useState(null);
  ...
  const reportUpdateStatus = React24.useCallback((snapshot) => {
    setRunningVersion(snapshot.runningVersion);
  }, []);
  const rpcCalls = React24.useMemo(() => createLoopbackAwareRpcCalls({
    dingtalkRpcCall,
    discordRpcCall,
    feishuRpcCall,
    qqRpcCall,
  ...
  const botSettingsContext = React24.useMemo(() => Object.freeze({
