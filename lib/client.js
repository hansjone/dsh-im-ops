...
function PhoneTypeahead({
  ...
  const suggestions = (contacts ?? []).filter((contact) => {
    ...
    return (contact.phone ?? "").includes(q.replace(/[^\d]/g, "")) || (contact.pushName ?? "").toLowerCase().includes(q);
  }).slice(0, 8);
  return h2(
    "div",
    ...
    suggestions.length === 0 ? null : h2(
      "ul",
      { className: "dim-accessSuggestList" },
...
function MemberRows({
  ...
  return h2(
    "fieldset",
    { className: "dim-accessScene", disabled },
    h2("legend", null, title),
...
function AdminPhones({
  ...
  return h2(
    ...
    { className: "dim-accessScene", disabled },
    h2("legend", null, title),
    help ? h2("p", { className: "dim-accessUsersEmpty" }, help) : null,
  ...
  return h2(
      ...
      (draft.pending ?? []).filter((entry) => entry.status === "pending" || !entry.status).length === 0 ? h2("div", { className: "dim-accessUsersEmpty" }, "\u6682\u65E0\u5F85\u6279\u7533\u8BF7") : h2(
        ...
        (draft.pending ?? []).filter((entry) => entry.status === "pending" || !entry.status).map((entry) => h2(
          ...
          h2(
            "div",
            { className: "dim-accessField" },
            h2("span", null, entry.kind === "group" ? ["\u7FA4\u804A", " ", entry.groupJid].join("") : "\u79C1\u804A"),
    ...
    h2(
      ...
      knownGroupJids.length === 0 ? h2(
        ...
        return h2(
          "div",
          { key: groupJid, className: "dim-accessScene", "data-scene": "group-grant" },
          h2("h3", null, group.title || groupJid),
          h2(
            "label",
            { className: "dim-accessField" },
            h2("span", null, "\u7FA4\u5907\u6CE8\u540D"),
